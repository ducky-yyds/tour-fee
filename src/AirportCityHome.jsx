...
export default function AirportCityHome({ city, onBack, onAdd }) {
  ...
  return <main className="airport-city-page page-width">
    ...
    <div className="airport-coverage-note"><Info size={19}/><p>这里已收录机场位置与交通规划入口。景点、酒店和当地食宿价格仍待整理；加入路线后可自行添加景点与预算，系统会保留待补充提示。机场名录中的定期航班标记不代表当前航线、班期或可订座位。</p></div>
    <section className="airport-directory"><div className="content-heading"><div><h2>从机场，认识这一站</h2><p>具体出发日期、机场接驳与航班状态，请在预订前核对。</p></div></div>
      ...
      <div className="airport-list">{result?.airports?.map(airport => <article className="panel" key={airport.id}><div className="airport-code">{airport.iata || airport.icao || airport.ident}</div><div><h3>{airport.name}</h3><p>{TYPES[airport.type] || airport.type} · {airport.scheduledService ? '有定期航班记录' : '未记录定期客运'}</p><div className="airport-links"><OutLink href={`https://www.google.com/travel/flights?q=${encodeURIComponent(`Flights to ${airport.iata || airport.name}`)}`}>查询机票</OutLink>{airport.homeUrl && /^https?:\/\//.test(airport.homeUrl) && <OutLink href={airport.homeUrl}>机场官网</OutLink>}<OutLink href={airport.sourceUrl || city.sourceUrl}>查看资料来源</OutLink></div></div></article>)}</div>
