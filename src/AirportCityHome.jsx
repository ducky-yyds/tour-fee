...
export default function AirportCityHome({ city, onBack, onChooseDestination }) {
  ...
  return <main className="airport-city-page page-width">
    ...
    <section className="airport-city-hero">
      <div><span className="eyebrow">TRANSPORT INFORMATION</span><h1>{city.name}</h1><p>{city.nameEn !== city.name ? `${city.nameEn} · ` : ''}{city.country}{city.subdivision ? ` · ${city.subdivision}` : ''}</p><div className="airport-city-tags"><span><Plane size={15}/>交通资料</span><span><MapPin size={15}/>{city.airportCount || city.airportIds?.length || 1} 处机场设施</span></div><button className="primary-button" onClick={onChooseDestination}>选择旅行目的地 <ArrowRight size={17}/></button></div>
    ...
    <div className="airport-coverage-note"><Info size={19}/><p>这是已有项目中的交通地点记录，保留供你核对机场和接驳。旅行目的地请从城市列表选择；此处没有确认的当地食宿报价。</p></div>
