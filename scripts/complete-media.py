...
def nearby(cities, items, media, excluded):
    def photo_pool(center, radius=10000, limit=250):
        ...
        return photos
    for city in cities:
        targets = [item for item in items if item['_city']['id'] == city['id'] and item['_kind'] == 'place' and item['id'] not in excluded and (not valid_photo(media['attractions'].get(item['id'])) or media['attractions'][item['id']].get('scope') == 'illustration')]
        if not targets:
            continue
        try:
...
def fallback(items, media):
    for item in items:
        ...
        if item.get('imageRef') != item['id'] and valid_photo(reference) and reference.get('scope') != 'illustration' and file_title(reference.get('fileTitle')) not in NEARBY_EXCLUSIONS:
